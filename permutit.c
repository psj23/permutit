...
int main(int argc, char *argv[])
{
	...
	if (c != 0) {
		...
		cleanup(opts, args, nl, (c == -2) ? (MALmsg) : (NULL));
	}
	for (u = opts; *u != NULL; ++u)
		if (**u == 'I' || **u == 'i') break;
	if (*u != NULL) {
...
static void lptnfo(char *s) {
	...
	fprintf(stderr, " Print all permutations of 7 objects:\n");
	fprintf(stderr, "%s argA argB argC argD argE argF argG\n", s);
	fprintf(stderr, " Print all permutations of 6 objects, 5 at a time:\n");
	fprintf(stderr, "%s %cr5 argA argB argC argD argE argF\n", s, optdlm);
	fprintf(stderr, " Print all permutations of 5 objects, sorted:\n");
	fprintf(stderr, "%s %cs argB argC argA argE argD\n", s, optdlm);
	fprintf(stderr, " Print all combinations of 5 objects, 3 at a time:\n");
	fprintf(stderr, "%s %cc %cr3 argA argB argC argD argE\n",\
	 s, optdlm, optdlm);
	fputc('\n', stderr);
	fprintf(stderr, "Options, if any, must always be specified\n");
	fprintf(stderr, "before positional arguments.\n");
	return;
}
