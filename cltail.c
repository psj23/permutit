...
char *MAEmsg = "calloc(): Memory allocation error.";
	...
	*args = calloc(nargs + 1, sizeof (char *));
	...
	while (argv[optind] != NULL) {
		...
		if (c == -1) {
			...
			(optarg != NULL) ? (y = strlen(optarg)) : (y = 0);
			*p = calloc(y + 2, sizeof (**p));
			if (*p == NULL) {
				fprintf(stderr, "%s\n", MAEmsg);
				return MLCERR;
			}
			...
			*(*p + 1) = '\0';
			...
			else if (i == optind && optdlm == '/') {
				++p;
				optdos(argc, argv, optstr, &p);
			}
			else
				++p;
		}
